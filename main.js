...
function syncToGCal() {
    ...
    for (let i = 0; i < response_data.results.length; i++) {
        ...
        if (CALENDAR_IDS[calendar_name] && calendar_id && event.id) {
            ...
            let modified_eId;
            if (deleteEvent(event.id, calendar_id) && (modified_eId = createEvent(result, event, calendar_name))) {
            ...
            console.log('[+GC] Event %s failed to move to %s.', event.id, calendar_name);
        ...
        console.log('[+GC] Calendar name %s not found in dictionary. Aborting sync.', calendar_name);
    }
...
function parseEvents(events, ignored_eIds) {
    ...
    for (let i = 0; i < events.items.length; i++) {
        ...
        if (event.start.date) {
            ...
            console.log('[+ND] Event found %s %s (%s)', event.id, event.summary, start.toLocaleString());
        }
        ...
        try {
            ...
            if (err instanceof InvalidEventError && SKIP_BAD_EVENTS) {
                console.log('[+ND] Skipping creation of event %s due to invalid properties.', event.id);
...
function getPageProperty(result, property) {
    ...
    try {
        ...
        const url = 'https://api.notion.com/v1/pages/' + page_id + '/properties/' + property_id;
